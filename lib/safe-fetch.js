...
function isPrivateAddress(addr) {
  ...
  if (net.isIPv6(addr)) {
    ...
    // IPv4-mapped IPv6 — decide based on the embedded v4 address.
    ...
    // Link-local fe80::/10 — first 16 bits are fe80..febf.
    if (/^fe[89ab]/.test(lower)) return true;
    // Multicast ff00::/8 — first byte 0xff.
    if (/^ff/.test(lower)) return true;
    return false;
  }
  return true; // not an IP literal — caller should resolve through safeLookup
}
